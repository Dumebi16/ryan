...
const StrategyIcon = ({ className }: { className?: string }) => {
  const trunk = Array.from({ length: 5 }, (_, i) => [80, 12 + i * 10] as [number, number]);
    ...
    <>
      <section className="lg:hidden relative bg-[#F5F5F0] border-t border-black/8 px-6 sm:px-10 pt-20 pb-20" style={{ backgroundImage: "radial-gradient(ellipse 65% 55% at 22% 65%, rgba(212,175,55,0.11) 0%, transparent 60%), radial-gradient(ellipse 55% 45% at 78% 35%, rgba(212,175,55,0.07) 0%, transparent 60%)" }}>
        <div className="mb-14">
          ...
          <h2 className="text-[1.75rem] sm:text-[2rem] font-medium text-black leading-tight tracking-tight mb-5">A Clear Pathto Funding</h2>
      ...
      <section className="hidden lg:flex relative bg-[#F5F5F0] border-t border-black/8 h-screen overflow-hidden" style={{ backgroundImage: "radial-gradient(ellipse 65% 55% at 22% 65%, rgba(212,175,55,0.11) 0%, transparent 60%), radial-gradient(ellipse 55% 45% at 78% 35%, rgba(212,175,55,0.07) 0%, transparent 60%)" }}>
        <div className="shrink-0 w-[42%] flex flex-col justify-center pl-24 pr-10 z-10">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }} viewport={{ once: true }}>
            <span className="font-kiona text-[9px] text-black/35 block mb-6">PROCESS</span>
            <h2 className="text-[1.75rem] md:text-[2rem] xl:text-[2.5rem] font-medium text-black leading-tight tracking-tight mb-5">A Clear Pathto Funding</h2>
        ...
        <div ref={rightColRef} className="flex-1 h-full overflow-y-scroll" style={{ scrollbarWidth: "none" } as object}>
  ...
  return (
    <section className="bg-black border-t border-white/[0.06] px-6 sm:px-10 lg:px-24 pt-24 md:pt-32 pb-28 md:pb-40">
      ...
      <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} transition={{ duration: 0.6, delay: 0.2 }} viewport={{ once: true, amount: 0.3 }} className="flex items-center justify-between max-w-4xl mt-16 md:mt-20">
        <div className="flex items-center gap-8"><div className="flex items-center gap-3">{TESTIMONIALS.map((_, i) => (<button key={i} onClick={() => handleChange(i)} aria-label={`Show testimonial ${i + 1}`} aria-current={i === active} className="group py-4"><span className={`block h-px transition-all duration-500 ease-out ${i === active ? "w-12 bg-[#D4AF37]" : "w-6 bg-white/[0.18] group-hover:w-9 group-hover:bg-white/[0.38]"}`} /></button>))}</div><span className="font-kiona text-[8px] text-white/22 tracking-widest">{String(active + 1).padStart(2, "0")} / {String(TESTIMONIALS.length).padStart(2, "0")}</span></div>
...
const FAQ_ITEMS = [
  { question: "What types of businesses do you help?", answer: "Ryan works with business owners across most industries — from service businesses and franchises to manufacturing and professional practices. Whether you are expanding an existing operation, purchasing a business, or refinancing, the focus is on finding the right SBA structure for your specific situation." },
  { question: "How long does SBA financing usually take?", answer: "Most SBA transactions move to closing in 60 to 90 days from a complete application. Timelines vary based on deal complexity and documentation readiness. Ryan provides initial qualification guidance within 24 hours so you understand your path before the process formally begins." },
  { question: "What do I need to qualify?", answer: "Lenders typically evaluate business financials, personal credit, industry experience, and the overall strength of the opportunity. Requirements depend on the loan purpose and deal structure. Ryan reviews the key factors upfront and gives you honest feedback on where you stand before any formal submission." },
  { question: "Can you help with business acquisitions?", answer: "Yes. Business acquisition is a core focus. Ryan guides buyers through the full SBA financing process — from understanding what lenders look for, to navigating deal structure, seller notes, and equity injection requirements. The goal is to position the deal correctly from the start." },
];
      ...
      <div className="flex flex-col lg:flex-row gap-16 lg:gap-20">
        <div className="lg:w-[28%] shrink-0">
          <motion.div initial={{ opacity: 0, y: 12 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.55 }} viewport={{ once: true, amount: 0.3 }}>
            ...
            <Link to="/contact" className="font-kiona text-[11px] border border-white/20 text-white/75 px-8 py-3.5 hover:bg-white/[0.07] hover:border-white/35 hover:text-white transition-all duration-300 inline-flex items-center gap-3">CONTACT RYAN <ArrowRight size={12} strokeWidth={1.5} /></Link>
          </motion.div>
        </div>
        <div className="flex-1 border-t border-white/[0.10]">
          {FAQ_ITEMS.map((item, i) => (
            <div key={i} className="border-b border-white/[0.10]">
              <button onClick={() => setOpen(open === i ? null : i)} className="group w-full flex items-start justify-between gap-6 py-6 text-left cursor-pointer px-2 -mx-2 rounded-sm transition-all duration-300" style={{ background: "transparent" }}>
              ...
              <AnimatePresence initial={false}>{open === i && (<motion.div key="answer" initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.32, ease: [0.25, 0.1, 0.25, 1] }} className="overflow-hidden"><p className="pb-7 px-0 text-[15px] text-white/45 leading-relaxed max-w-xl">{item.answer}</p></motion.div>)}</AnimatePresence>
            </div>
          ))}
        </div>
      </div>
  ...
  return (
    <section className="relative bg-black overflow-hidden pb-28 md:pb-40">
      ...
      <div className="relative z-10 px-6 sm:px-10 lg:px-24 pt-16 md:pt-24 max-w-4xl mx-auto text-center">
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.55, delay: 0.18 }} viewport={{ once: true, amount: 0.3 }}><Link to="/business-acquisition" className="font-kiona text-[11px] border border-white/20 text-white px-10 py-4 hover:bg-white hover:text-black hover:border-white transition-all duration-300 inline-flex items-center gap-3">EXPLORE BUSINESS ACQUISITION <ArrowRight size={12} strokeWidth={1.5} /></Link></motion.div>
      </div>
    </section>
  );
};
  ...
  return (
    <section className="relative bg-black overflow-hidden px-6 sm:px-10 py-28 md:py-40">
      ...
      <div className="relative z-10 max-w-2xl mx-auto text-center">
        <motion.div initial={{ opacity: 0, scaleX: 0.4 }} whileInView={{ opacity: 1, scaleX: 1 }} transition={{ duration: 0.55, ease: "easeOut" }} viewport={{ once: true, amount: 0.4 }} className="flex items-center justify-center gap-2.5 mb-10"><span className="block h-px w-8 bg-white/50" /><span className="block w-1.5 h-1.5 rounded-full bg-white/70" /><span className="block h-px w-8 bg-white/50" /></motion.div>
        <motion.span initial={{ opacity: 0, y: 10 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.05 }} viewport={{ once: true, amount: 0.4 }} className="font-kiona text-[9px] text-white/30 block mb-8 tracking-widest">GET STARTED</motion.span>
        <motion.h2 initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.1 }} viewport={{ once: true, amount: 0.4 }} className="text-[2.25rem] sm:text-[3rem] md:text-[3.75rem] font-medium text-white leading-[1.08] tracking-tight mb-7">Ready to Explore Your Funding Options?</motion.h2>
        <motion.p initial={{ opacity: 0, y: 14 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.65, delay: 0.17 }} viewport={{ once: true, amount: 0.4 }} className="text-[15px] md:text-base text-white/45 leading-relaxed mb-12 max-w-md mx-auto">Whether you are acquiring a business, preparing for growth, or evaluating SBA financing, Ryan can help you understand the next move.</motion.p>
        <motion.div initial={{ opacity: 0, y: 12 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.55, delay: 0.24 }} viewport={{ once: true, amount: 0.4 }} className="flex flex-col sm:flex-row items-center justify-center gap-4"><Link to="/contact" className="font-kiona text-[11px] bg-[#D4AF37] text-black px-10 py-4 hover:bg-white hover:text-black transition-colors duration-300 inline-flex items-center gap-3 w-full sm:w-auto justify-center">START YOUR APPLICATION <ArrowRight size={12} strokeWidth={1.5} /></Link><a href="https://cal.com/ryan-kroge-nsvqdg/30min" target="_blank" rel="noopener noreferrer" className="font-kiona text-[11px] border border-white/20 text-white px-10 py-4 hover:bg-white hover:text-black hover:border-white transition-all duration-300 inline-flex items-center gap-3 w-full sm:w-auto justify-center">BOOK A CALL</a></motion.div>
      </div>
    </section>
  );
};
  ...
  return (
    <>
      ...
      <section id="services" className="relative z-10 bg-[#F5F5F0] py-20 md:py-28 px-6 sm:px-10 md:px-16 lg:px-24" style={{ backgroundImage: "radial-gradient(ellipse 65% 55% at 22% 65%, rgba(212,175,55,0.11) 0%, transparent 60%), radial-gradient(ellipse 55% 45% at 78% 35%, rgba(212,175,55,0.07) 0%, transparent 60%)" }}>
        <div className="max-w-7xl mx-auto">
          <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-10 mb-16 lg:mb-20">
            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }} viewport={{ once: true }} className="max-w-2xl"><span className="font-kiona text-[9px] text-black/35 block mb-6">CAPABILITIES</span><h2 className="text-[1.75rem] sm:text-[2rem] md:text-[2.5rem] lg:text-[2.75rem] font-medium text-black leading-tight tracking-tight mb-5">Strategic Funding Solutions<br className="hidden lg:block" />{" "}for Growth, Acquisition,<br className="hidden lg:block" />{" "}and Long-Term Success</h2><p className="text-sm md:text-[15px] text-black/60 leading-relaxed max-w-lg">Ryan works with business owners who need more than a loan product. He provides financing guidance, deal support, and strategic insight tailored to the stage and structure of the opportunity.</p></motion.div>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 md:gap-6">
            {[
              { id: "sba-loans", path: "/sba-loans", title: "SBA Loans", tag: "SBA 7(a) & 504", description: "Funding solutions for acquisitions, expansion, partner buyouts, equipment, and working capital.", Icon: SBALoansIcon },
              { id: "business-acquisition", path: "/business-acquisition", title: "Business Acquisition", tag: "Buy-Side Advisory", description: "Guidance for buyers navigating valuation, structure, lender expectations, and the financing process.", Icon: AcquisitionIcon },
              { id: "strategic-guidance", path: "/strategic-financial-guidance", title: "Strategic Financial Guidance", tag: "Deal Strategy", description: "Clarity around loan readiness, financial positioning, and the decisions that shape a stronger funding outcome.", Icon: GuidanceIcon }
            ].map(({ id, path, title, tag, description, Icon }, i) => (
              <motion.div key={title} id={id} initial={{ opacity: 0, y: 18 }} whileInView={{ opacity: 1, y: 0 }} whileHover={{ scale: 1.025, y: -4 }} whileTap={{ scale: 0.98 }} transition={{ default: { duration: 0.6, delay: i * 0.1 }, scale: { type: "spring", stiffness: 260, damping: 22 }, y: { type: "spring", stiffness: 260, damping: 22 } }} viewport={{ once: true }} className="bg-white/[0.18] backdrop-blur-xl rounded-2xl border border-white/50 shadow-[0_4px_24px_rgba(0,0,0,0.06),_0_1px_2px_rgba(0,0,0,0.04),_inset_0_1px_0_rgba(255,255,255,0.92),_inset_0_-1px_0_rgba(0,0,0,0.04)] hover:shadow-[0_16px_48px_rgba(0,0,0,0.1),_0_2px_4px_rgba(0,0,0,0.06),_inset_0_1px_0_rgba(255,255,255,0.92)] transition-shadow duration-300 cursor-pointer flex flex-col p-7 sm:p-9 sm:min-h-[440px] lg:min-h-[460px]">
                <Link to={path} className="flex flex-col h-full w-full">
                  <div className="flex items-start justify-between mb-6 min-h-[3.5rem]"><h3 className="font-medium text-[1rem] md:text-[1.05rem] text-black leading-snug max-w-[70%]">{title}</h3><span className="font-kiona text-[7px] text-black/28 text-right leading-relaxed mt-0.5 shrink-0 max-w-[28%]">{tag}</span></div>
                  <div className="flex-1 flex items-center justify-center py-4"><Icon className="w-full max-w-[180px] text-black/18" /></div>
                  <div className="mt-auto pt-6 border-t border-black/8"><p className="text-[13px] md:text-sm text-black/55 leading-relaxed">{description}</p><div className="flex items-center gap-2 mt-5"><span className="font-kiona text-[8px] text-black/32">LEARN MORE</span><ArrowRight size={10} strokeWidth={1.5} className="text-black/32" /></div></div>
                </Link>
              </motion.div>
            ))}
          </div>
      ...
      <section className="relative z-10 bg-[#F5F5F0] border-t border-black/8 py-20 md:py-28 px-6 sm:px-10 md:px-16 lg:px-24" style={{ backgroundImage: "radial-gradient(ellipse 65% 55% at 22% 65%, rgba(212,175,55,0.11) 0%, transparent 60%), radial-gradient(ellipse 55% 45% at 78% 35%, rgba(212,175,55,0.07) 0%, transparent 60%)" }}>
        <div className="max-w-7xl mx-auto">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }} viewport={{ once: true }} className="text-center max-w-2xl mx-auto mb-16 md:mb-20">
            <span className="font-kiona text-[9px] text-black/35 block mb-6">WHY RYAN</span>
            <h2 className="text-[1.75rem] sm:text-[2rem] md:text-[2.5rem] lg:text-[2.75rem] font-medium text-black leading-tight tracking-tight">A More Strategic<br />Lending Partner</h2>
          </motion.div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 md:gap-6">
            {[ { title: "Deep SBA Expertise", description: "More than two decades of lending and business experience applied to real financing decisions.", Icon: ExpertiseIcon }, { title: "Clear Guidance", description: "Complex funding conversations simplified into practical next steps.", Icon: GuidanceSmallIcon }, { title: "Acquisition Insight", description: "Support that goes beyond loan placement, especially when a business purchase is involved.", Icon: InsightIcon }, { title: "Responsive Process", description: "Fast communication, honest feedback, and a client-first approach from first conversation to closing.", Icon: ProcessSmallIcon } ].map(({ title, description, Icon }, i) => (
              <motion.div key={title} initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} whileHover={{ scale: 1.025, y: -4 }} whileTap={{ scale: 0.98 }} transition={{ default: { duration: 0.6, delay: i * 0.09 }, scale: { type: "spring", stiffness: 260, damping: 22 }, y: { type: "spring", stiffness: 260, damping: 22 } }} viewport={{ once: true }} className="bg-white/[0.18] backdrop-blur-xl rounded-2xl border border-white/50 shadow-[0_4px_24px_rgba(0,0,0,0.06),_0_1px_2px_rgba(0,0,0,0.04),_inset_0_1px_0_rgba(255,255,255,0.92),_inset_0_-1px_0_rgba(0,0,0,0.04)] hover:shadow-[0_16px_48px_rgba(0,0,0,0.1),_0_2px_4px_rgba(0,0,0,0.06),_inset_0_1px_0_rgba(255,255,255,0.92)] transition-shadow duration-300 cursor-pointer flex flex-col p-7 sm:p-8">
                <div className="w-11 h-11 flex items-center justify-center mb-7 shrink-0"><Icon className="w-[22px] h-[22px] text-black/30" /></div>
                <div className="flex items-start gap-3 mb-4"><span className="font-kiona text-[8px] text-black/22 mt-1 shrink-0 leading-none">0{i + 1}</span><h3 className="font-medium text-black text-[0.95rem] md:text-[1rem] leading-snug">{title}</h3></div>
                <p className="text-[13px] md:text-sm text-black/50 leading-relaxed pl-6">{description}</p>
              </motion.div>
            ))}
          </div>
          <motion.div initial={{ opacity: 0, y: 14 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.2 }} viewport={{ once: true }} className="mt-16 md:mt-20 flex flex-col sm:flex-row items-start sm:items-center gap-6 border-t border-black/8 pt-10"><div className="w-8 h-px bg-[#D4AF37] shrink-0 mt-1 hidden sm:block" /><blockquote className="text-[0.9rem] md:text-[0.95rem] text-black/45 italic leading-relaxed font-light max-w-xl">"Extremely knowledgeable and passionate, Ryan is on top of his game."</blockquote><span className="font-kiona text-[7.5px] text-black/25 sm:ml-auto shrink-0">CLIENT TESTIMONIAL</span></motion.div>
        </div>
      </section>

      <ProcessSection />

      <div className="w-full h-28 sm:h-36 pointer-events-none" style={{ background: "radial-gradient(ellipse 180% 70% at 50% 60%, rgba(0,0,0,0.12) 0%, transparent 100%), linear-gradient(to bottom, #F5F5F0 0%, #F5F5F0 5%, #e8e8e3 12%, #d0d0cb 22%, #a8a8a3 34%, #6a6a65 47%, #2d2d2a 60%, #0d0d0a 74%, #000000 88%)" }} />
      ...
      <div className="fixed right-8 bottom-24 hidden lg:block">
        <span className="font-kiona text-[9px] text-white/20 vertical-text">EST. 2026 — FUNDING THE FUTURE</span>
      </div>
    </>
  );
}
